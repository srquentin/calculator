...
initCalcCmd()
{
      ...
      cerr << "Registering \"initCal\" commands fails... exiting" << endl;
      return false;
   }
   return true;
}
...
GNAddCmd::exec(const string& option)
{
   ...
   // check option 1
    ...
    for (int i = 1; i < 3; i++)
    {
        if (!GNum::getStrVal(options[i], v[i]))
           return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[i]);
    }
    v[0] = v[1] + v[2];
    GNum::setVarVal(options[0], v[0]);
    cout << options[0] << "(" << v[0] << ") = "<< v[1] << " + " << v[2] <<endl;
    return CMD_EXEC_DONE;
}
